...
// -
func fverifyRequest(ctx *fasthttp.RequestCtx) {
	start := time.Now()
	...
	x5u, hh, err := fvalidateHeader(ctx, traceID, clientIP, token[0])
	if err != nil {
	...
	orderedMap, iatInClaims, err := fvalidateClaims(ctx, traceID, clientIP, token[0], origTN, destTNs, iat, start.Unix())
	if err != nil {
		// function writes to http.ResponseWriter directly
		return
	}
...
func fvalidateHeader(ctx *fasthttp.RequestCtx, traceID, clientIP, j string) (string, map[string]interface{}, error) {
	var x5u string
	s := strings.Split(j, ".")
...
// check if expected key-values exist
func fvalidateClaims(ctx *fasthttp.RequestCtx, traceID, clientIP, j, oTN string, dTNs []string, iat, t int64) (map[string]interface{}, int64, error) {
